...
<?php
$hire_car = $admin->get_all_hire_bus('car hire');
if ($hire_car->num_rows > 0) {
    $hire_cars_arr = array();
    while ($row = $hire_car->fetch_assoc()){
        $hire_cars_arr[] = array(
...
<div class="content">
    <div class="col-md-12">
        <h4 class="card-title text-center">Car Hire</h4>
        <div class="card">
            ...
            <div class="card-body">
                <table id="datatable" class="table table-striped table-bordered" style="width: 100%" cellspacing="0">
                    ...
                    if (!isset($message)){
                        $n=0;
                        foreach ($hire_cars_arr as $booking){
                            ?>
                            <tr>
                                <td><?= ++$n; ?></td>
                                <td><?= $booking['email']; ?></td>
...
<script>
    $(document).on("click", "#hire_car", function () {
